...
void generate_expression(FILE *fptr, T_expression side, int reg) {
    switch (side->lhs->type) { //left side of expression (num or var)
            case NUM: 
                fprintf(fptr, "\tmovss\t.LC%d(%%rip), %%xmm%d\n", curr_const++, reg);
                break;
            case VAR:
                fprintf(fptr, "\tmovss\t%s(%%rip), %%xmm%d\n", side->lhs->value.var, reg);
    }
...
void make_start(FILE *fptr) {
    fprintf(fptr, "\tpushq\t%%rbp\n");
    fprintf(fptr, "\tmovq\t%%rsp, %%rbp\n");
}
